...
set(_exported_dependencies "rclcpp;rclcpp_components;cv_bridge;sensor_msgs;tf2_geometry_msgs;tf2_ros;tf2_msgs;geometry_msgs;nav_msgs;visualization_msgs;isaac_ros_visual_slam_interfaces")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to isaac_ros_visual_slam_DEFINITIONS, isaac_ros_visual_slam_INCLUDE_DIRS,
# isaac_ros_visual_slam_LIBRARIES, and isaac_ros_visual_slam_LINK_FLAGS.
# Additionally collect the direct dependency names in
# isaac_ros_visual_slam_DEPENDENCIES as well as the recursive dependency names
# in isaac_ros_visual_slam_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(isaac_ros_visual_slam_DEPENDENCIES ${_exported_dependencies})
  set(isaac_ros_visual_slam_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(isaac_ros_visual_slam_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(isaac_ros_visual_slam_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(isaac_ros_visual_slam_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(isaac_ros_visual_slam_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(isaac_ros_visual_slam_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND isaac_ros_visual_slam_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
