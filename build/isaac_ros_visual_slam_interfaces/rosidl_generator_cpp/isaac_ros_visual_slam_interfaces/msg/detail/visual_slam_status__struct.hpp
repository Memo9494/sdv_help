...
struct VisualSlamStatus_
{
  ...
  using _integrator_state_type =
    uint8_t;
  _integrator_state_type integrator_state;
  ...
  Type & set__integrator_state(
    const uint8_t & _arg)
  {
    this->integrator_state = _arg;
    return *this;
  }
  ...
  bool operator==(const VisualSlamStatus_ & other) const
  {
    ...
    if (this->integrator_state != other.integrator_state) {
      return false;
    }
